...
The camera rides **close behind the marble** (a third-person chase rig that leans
into your heading), so steering is **camera-relative**: push **up / forward** to
roll away from the camera, left/right to carve.

**Controls**
| | |
|---|---|
| **Desktop** | `W`/`↑` forward · `A`·`S`·`D` to steer · hold **Shift** to boost |
...
| `src/config.js` | **All tunables**: biome worlds, skins, trails, power-ups, economy, pacing, director constants, plus the `CAMERA` chase-rig + `FX` (bloom/streaks) knobs |
...
| `src/game.js` | The 3D engine: scene, marble physics, every hazard/entity, power-ups, particles, trail, the close **chase camera** + camera-relative steering, and the **post-FX** chain (bloom → tone-map → colour-grade) with sparks & ground speed-streaks |
...
| `vendor/three.module.min.js` | Three.js r160, vendored locally (no external CDN) |
| `vendor/jsm/**` | Three.js r160 postprocessing addons (EffectComposer, UnrealBloom, OutputPass, …), vendored for the bloom/FX chain |
